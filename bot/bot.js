...
function scheduleReconnect(reason = 'connection ended') {
  ...
  reconnectTimer = setTimeout(() => { reconnectTimer = null; if (!shuttingDown) createBot(); }, delay);
}
...
function dnsProbe() {
  return new Promise(resolve => {
    ...
    dns.lookup(config.serverHost, { all: true }, (err, addresses) => {
      ...
      console.log(`✅ [DNS] ${addresses.map(a => a.address).join(', ')}`); resolve(true);
    });
  });
}
function tcpProbe() {
...
async function createBot() {
  ...
  if (!(await preflight())) { console.log('⏳ [RETRY] Server unreachable. I will keep trying.'); scheduleReconnect('preflight failed'); return; }
  try {
  ...
  } catch (err) { bot = null; console.log(`❌ [CREATE] ${err.code || 'ERROR'}: ${err.message}`); scheduleReconnect('createBot error'); return; }
  let lastEntityTime = Date.now();
  function lookAround() { if (!bot?.entity) return; try { bot.look(bot.entity.yaw + (Math.random() - 0.5) * 1.5, (Math.random() - 0.5) * 0.35, true); } catch (_) {} }
  ...
  function danceStep() {
    ...
    for (const state of ['forward','back','left','right']) { try { bot.setControlState(state, false); } catch (_) {} }
    const [direction,duration] = danceSteps[danceIndex]; danceIndex=(danceIndex+1)%danceSteps.length;
    try { bot.setControlState(direction,true); if(chance(35)){bot.setControlState('jump',true);setTimeout(()=>{try{bot.setControlState('jump',false)}catch(_){}} ,220)} lookAround(); if(chance(35)) bot.swingArm(); } catch (_) {}
    console.log(`💃 [DANCE] ${direction} ${duration}ms`);
    movementTimer=setTimeout(danceStep,duration);
  }
  const armorSlots={head:['netherite_helmet','diamond_helmet','iron_helmet','golden_helmet','chainmail_helmet','leather_helmet'],torso:['netherite_chestplate','diamond_chestplate','iron_chestplate','golden_chestplate','chainmail_chestplate','leather_chestplate'],legs:['netherite_leggings','diamond_leggings','iron_leggings','golden_leggings','chainmail_leggings','leather_leggings'],feet:['netherite_boots','diamond_boots','iron_boots','golden_boots','chainmail_boots','leather_boots']};
  let armorBusy=false;
  async function equipArmor(){if(!bot?.entity||armorBusy)return;armorBusy=true;try{for(const [slot,items] of Object.entries(armorSlots)){for(const itemName of items){const item=bot.inventory.items().find(i=>i.name===itemName);if(!item)continue;try{await bot.equip(item,slot);console.log(`🛡️ [ARMOR] ${itemName} → ${slot}`)}catch(_){}break}}}finally{armorBusy=false}}
  function watchdog(){if(!bot||shuttingDown)return;if(!bot.entity){console.log('⚠️ [WATCHDOG] No entity; reconnecting.');try{bot.quit('Watchdog reconnect')}catch(_){}scheduleReconnect('watchdog: no entity')}else if(Date.now()-lastEntityTime>120000){console.log('⚠️ [WATCHDOG] Entity inactive for 120s; reconnecting.');try{bot.quit('Watchdog timeout')}catch(_){}scheduleReconnect('watchdog timeout')}}
  bot.once('login',()=>console.log(`🔐 [LOGIN] Login accepted by server as ${usernameForAttempt}.`));
  bot.once('spawn',()=>{reconnectAttempt=0;startedAt=Date.now();lastEntityTime=Date.now();spawnTimer=clearTimeoutSafe(spawnTimer);console.log(`🎉 [SPAWN] SUCCESS — ${usernameForAttempt} joined ${config.serverHost}:${config.serverPort}`);if(bot.entity?.position)console.log(`📍 [POSITION] ${bot.entity.position.x.toFixed(1)}, ${bot.entity.position.y.toFixed(1)}, ${bot.entity.position.z.toFixed(1)}`);if(danceEnabled){console.log('💃 [DANCE] continuous dance started');danceIndex=0;movementTimer=clearTimeoutSafe(movementTimer);danceStep()}armorTimer=setTimeout(equipArmor,4000);armInterval=setInterval(()=>{if(bot?.entity&&chance(70))lookAround()},8000);chatInterval=setInterval(()=>{if(bot?.entity)console.log(`💚 [ALIVE] ${Math.floor((Date.now()-startedAt)/1000)}s | hp=${bot.health} | food=${bot.food}`)},5*60*1000);watchdogTimer=setInterval(watchdog,WATCHDOG_INTERVAL)});
  ...
  spawnTimer=setTimeout(()=>{if(!bot?.entity&&!shuttingDown){console.log(`⏱️ [SPAWN TIMEOUT] No spawn after ${SPAWN_TIMEOUT/1000}s.`);try{bot.quit('Spawn timeout')}catch(_){}scheduleReconnect('spawn timeout')}},SPAWN_TIMEOUT);
}
...
function setName(name){const n=String(name||'').replace(/[^A-Za-z0-9_]/g,'').slice(0,16);if(!n)throw new Error('Invalid username');nextBotUsername=n;config.botUsername=n;console.log(`👤 [NAME] Next connection username=${n}`);if(bot)reconnect('name changed')}
function setMovement(enabled){danceEnabled=Boolean(enabled);if(danceEnabled&&bot?.entity){movementTimer=clearTimeoutSafe(movementTimer);danceEnabled=true;console.log('💃 [DANCE] enabled');setTimeout(()=>{if(danceEnabled) { /* danceStep is scoped to createBot */ try{bot.setControlState('forward',true)}catch(_){}}},0)}else{console.log('🛑 [DANCE] disabled');stopMovement();movementTimer=clearTimeoutSafe(movementTimer)}}

function pollControl(){https.get(`${CONTROL_URL}?t=${Date.now()}`,res=>{let data='';res.on('data',c=>data+=c);res.on('end',()=>{try{const c=JSON.parse(data);if(!c||Number(c.id||0)<=lastControlId)return;lastControlId=Number(c.id);console.log(`🎛️ [CONTROL] ${c.action}`);if(c.action==='stop')stop('Web control');else if(c.action==='start')start();else if(c.action==='reconnect')reconnect('Web control');else if(c.action==='chat')chat(String(c.value||''));else if(c.action==='name')setName(c.value);else if(c.action==='movement')setMovement(c.value===true||c.value==='on'||c.value===1)}catch(e){console.log(`⚠️ [CONTROL] ${e.message}`)}})}).on('error',e=>console.log(`⚠️ [CONTROL] ${e.message}`))}
...
console.log('============================================================');
controlTimer=setInterval(pollControl,3000);pollControl();start();
module.exports={getBot:()=>bot,getConfig:()=>config,getStartedAt:()=>startedAt,start,stop,reconnect,chat,setName,setMovement};
